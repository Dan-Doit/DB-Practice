...
SELECT COUNT(*) FROM EM WHERE EM_STCODE = 'D002' AND EM_CODE = 'K001';

SELECT COUNT(*) AS ISAC FROM EM WHERE EM_STCODE = 'D002' AND EM_CODE = 'K001' AND EM_PWD = '0001';
SELECT HI_EMCODE, HI_ACCDATE, HI_STATE, HI_EMSTCODE FROM HI;
INSERT INTO HI(HI_EMCODE, HI_ACCDATE, HI_STATE, HI_EMSTCODE) VALUES ('',DEFAULT,'','');

CREATE OR REPLACE VIEW ACCINFO AS 
SELECT EM_STCODE AS STCODE, EM_CODE AS EMCODE, EM_NAME AS EMNAME, EM_LEVEL AS EMLEVEL, TO_CHAR(HI_ACCDATE,'YYYYMMDDHH24MISS') AS ACC
FROM HI INNER JOIN EM ON EM.EM_CODE = HI.HI_EMCODE AND HI.HI_EMSTCODE = EM.EM_STCODE;

SELECT STCODE, EMCODE, EMNAME, EMLEVEL, MAX(ACC) AS ACC
FROM ACCINFO
WHERE EMCODE = 'K001' AND STCODE = 'D002'
GROUP BY  STCODE, EMCODE, EMNAME, EMLEVEL;
SELECT * FROM HI;
DELETE FROM HI WHERE TO_CHAR(HI_ACCDATE,'YYYYMMDD') = '20201029'; 



SELECT * FROM EM;
GRANT INSERT,UPDATE ON EM TO GILDONG, DOOLY, HEEDONG,DOUNER;
INSERT INTO EM(EM_SRCODE, EM_CODE, EM_PWD, EM_NAME, EM_LEVEL) VALUES ("","","","","");

UPDATE EM SET EM_PWD = '0000' WHERE EM_CODE = 'G009'; 
COMMIT;
...
SELECT GOCODE, GONAME, GOPRICE, SUM(QTY) AS QTY, SUM(AMOUNT) AS AMOUNT
FROM HISGOINFO
WHERE TO_CHAR(ODCODE,'YYYYMMDD') = '20201016'
GROUP BY GOCODE, GONAME, GOPRICE;


SELECT GO_CODE AS GOCODE, GO_NAME AS GONAME, GO_PRICE AS PRICE, MIN(SC_EXPIRE) AS EXPIRE
FROM GO INNER JOIN SC ON GO.GO_CODE = SC.SC_GOCODE
WHERE GO_CODE = '1001' AND SYSDATE <= SC_EXPIRE
GROUP BY GO_CODE, GO_NAME, GO_PRICE;

SELECT * FROM ot;
SELECT * FROM OD;
INSERT INTO OD(OD_CODE, OD_EMSTCODE, OD_EMCODE, OD_CMCODE, OD_STATE) VALUES (TO_DATE(?,'YYYYMMDDHH24MISS'),?,?,?,?);
INSERT INTO OT(OT_ODCODE,OT_GOCODE,OT_QTY,OT_STATE) VALUES (TO_DATE(?,'YYYYMMDDHH24MISS'),"","","");

GRANT INSERT ON OT TO DOOLY,DOUNER,HEEDONG,GILDONG;
